													...
													foreach($tabFic as $nomfic) {
														if(substr($nomfic, -2, 2) !== '..' && substr($nomfic, -1, 1) !== '.') {	
															...
															if($doiTEI != "") {
																...
																$reqAPI = "https://api.archives-ouvertes.fr/search/?fq=title_t:%22".urlencode(trim($critere))."*%22".$special."&rows=10000&fl=halId_s,doiId_s,title_s,subTitle_s,docType_s";
															}else{
																$critere = $titTEI;
															...
															//Quand on a que le titre et pas le DOI, il faut trouver une correspondance exacte sur tout le titre pour considérer qu'il s'agit d'un doublon
															if($doiTEI != "") {
																$reqAPIC = "https://api.archives-ouvertes.fr/crac/hal/?fq=title_t:%22".urlencode(trim($critere))."*%22".$special."&rows=10000&fl=halId_s,doiId_s,title_s,subTitle_s,docType_s";
															}else{
