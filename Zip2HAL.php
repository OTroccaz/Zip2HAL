												...
												if(isset($_POST[$soumis])) {
													...
													foreach($tabFic as $nomfic) {
														if(substr($nomfic, -2, 2) !== '..' && substr($nomfic, -1, 1) !== '.') {	
															...
															if($datePub != "") {$special = "%20AND%20(publicationDateY_i:(".$datePub1."%20OR%20".$datePub."%20OR%20".$datePub2.")%20OR%20inPress_bool:true)";}else{$special = "";}
															...
															if($doiTEI != "") {
																...
																$reqAPI = "https://api.archives-ouvertes.fr/search/?fq=title_t:%22".trim($critere)."*%22".$special."&rows=10000&fl=halId_s,doiId_s,title_s,subTitle_s,docType_s";
															}else{
																$critere = $titTEI;
																$reqAPI = "https://api.archives-ouvertes.fr/search/?fq=title_t:%22".trim($titTEI)."*%22".$special."&rows=10000&fl=halId_s,doiId_s,title_s,subTitle_s,docType_s";
															...
															//Quand on a que le titre et pas le DOI, il faut trouver une correspondance exacte sur tout le titre pour considérer qu'il s'agit d'un doublon
															if($doiTEI != "") {
																$reqAPIC = "https://api.archives-ouvertes.fr/crac/hal/?fq=title_t:%22".trim($critere)."*%22".$special."&rows=10000&fl=halId_s,doiId_s,title_s,subTitle_s,docType_s";
															}else{
																$reqAPIC = "https://api.archives-ouvertes.fr/crac/hal/?fq=title_t:%22".trim($titTEI)."*%22".$special."&rows=10000&fl=halId_s,doiId_s,title_s,subTitle_s,docType_s";
