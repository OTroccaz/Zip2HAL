	...
	for($i = 0; $i < count($nomAff); $i++) {
		...
		foreach($tabCode as $test) {
			...
			if($cptCode <= $max && !in_array($test, $anepasTester)) {						
				$reqAff = "https://api.archives-ouvertes.fr/ref/structure/?q=structAcronym_t:".$test."%20OR%20structAcronym_sci:".$test."%20AND%20valid_s:(VALID%20OR%20OLD)".$special."&fl=docid,valid_s,name_s,type_s,country_s,acronym_s&sort=valid_s%20desc,docid%20asc";
		...
		if($trouve == 0) {
			...
			foreach($tabCode as $test) {
				...
				if($cptCode <= $max && !in_array($test, $anepasTester)) {
					...
					$reqAff = "https://api.archives-ouvertes.fr/ref/structure/?q=(name_t:".$test."%20OR%20code_t:".$test."%20OR%20structAcronym_t:".$test.")".$typeSpe."%20AND%20valid_s:(VALID%20OR%20OLD)".$special."&fl=docid,valid_s,name_s,type_s,country_s,acronym_s&sort=valid_s desc,docid asc";
					$reqAff = str_replace(" ", "%20", $reqAff);
					...
					if($numFound != 0) {			
						...
						if($special != "") {//Dans HAL, on signale le plus souvent des institutions étrangères, pas des labos
							if(strpos($special, "fr") !== false) {
								$reqAff = "https://api.archives-ouvertes.fr/ref/structure/?q=(name_t:".$test."%20OR%20code_t:".$test."%20OR%20structAcronym_t:".$test.")%20AND%20valid_s:(VALID%20OR%20OLD)".$special."&fl=docid,valid_s,name_s,type_s,country_s,acronym_s&sort=valid_s desc,docid asc";
								$reqAff = str_replace(" ", "%20", $reqAff);
		...
		//Mais, si pas de virgule ou nombre de virgules < 3, il faut naturellement conserver le dernier élément
			...
			foreach($tabCode as $test) {
				...
				if(count($tabCode) > 2) {$max = count($tabCode) - 2;}else{$max = count($tabCode);}
				if($cptCode <= $max && !in_array($test, $anepasTester)) {
					//$reqAff = "https://api.archives-ouvertes.fr/ref/structure/?q=(name_t:%22".$test."%22%20OR%20name_t:(".$test.")%20OR%20code_t:%22".$test."%22%20OR%20structAcronym_t:%22".$test."%22%20OR%20structAcronym_sci:%22".$test."%22)%20AND%20type_s:".$type."%20AND%20valid_s:(VALID%20OR%20OLD)&fl=docid,valid_s,name_s,type_s&sort=valid_s%20desc,docid%20asc";
